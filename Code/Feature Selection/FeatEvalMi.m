%FEATEVALMI Evaluation of feature set for classification
% Extension of prtools feateval to add mutual info option
%
% 	J = FEATEVAL(A,CRIT,T)
%   J = A*FEATEVAL([],CRIT,T)
%   J = A*FEATEVAL(CRIT,T)
...
function J = FeatEvalMi(varargin)
...
if mapping_task(argin,'definition')
    ...
    if isstr(crit)
        ...
        elseif strcmp(crit,'mi') % Mutual information
            if any(max(+a, [], 1) < 5)
                fprintf('FeatEvalMi WARNING: The dataset needs to be discretised into at least 10 integer bins\n')
            end
            J = mi(round(+a), getnlab(a));
        elseif strcmp(crit,'nmi') % Normalised mutual information
            if any(max(+a, [], 1) < 5)
                fprintf('FeatEvalMi WARNING: The dataset needs to be discretised into at least 10 integer bins\n')
            end
            J = mi(round(+a), getnlab(a));
            ...
            h2 = h(getnlab(a));
            J = J/min(h1, h2);
        elseif strcmp(crit,'jmi') % Joint Mutual information
            if any(max(+a, [], 1) < 5)
                fprintf('FeatEvalMi WARNING: The dataset needs to be discretised into at least 10 integer bins\n')
            end
			J = 0;
