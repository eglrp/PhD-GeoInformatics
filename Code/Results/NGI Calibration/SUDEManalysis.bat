...
echo Aspect
gdaldem aspect %DEMFILE% %ASPECTFILE% -trigonometric -alg Horn -zero_for_flat -co "COMPRESS=LZW"
gdaladdo -ro -r average --config COMPRESS_OVERVIEW LZW %ASPECTFILE% 2 4 8 16 32
echo Roughness
gdaldem roughness %DEMFILE% %ROUGHNESSFILE% -co "COMPRESS=LZW"
gdaladdo -ro -r average --config COMPRESS_OVERVIEW LZW %ROUGHNESSFILE% 2 4 8 16 32
pause
