...
heightFile = r"V:\Data\NGI\GEF DEM\3323d_2015_1001_GEF_DEM_Photoscan_hgt.tif"
...
if False:
    ...
    pylab.close('all')
    fig = pylab.figure()
    ScatterD(std, yc/1000., class_labels=classes, labels=None, thumbnails=thumbnails, regress=True, xlabel='log(mean(height))', ylabel='AGB (t/ha)')
    pylab.grid()

    # try only severe class - this area is easier to find ground because it is flatter and has less vegetation
    fig = pylab.figure()
    severeIdx = classes == 'Pristine'
    ScatterD(hgt[severeIdx], yc[severeIdx]/1000., class_labels=classes[severeIdx], labels=None, thumbnails=thumbnails[severeIdx], regress=True, xlabel='log(mean(height))', ylabel='AGB (t/ha)')
    pylab.grid()
