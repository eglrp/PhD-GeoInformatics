...
outFileName = 'C:\Data\Development\Projects\PhD GeoInformatics\Data\GEF Sampling\Sampling March 2018\Unknown Species.csv'
import csv
...
for ws in wb:
    ...
    outFileName = 'C:\Data\Development\Projects\PhD GeoInformatics\Data\GEF Sampling\Sampling March 2018\%s - Woody.csv' % (ws.title)
    with open(outFileName,'wb') as outfile:
...
for id, subPlot in subPlots.iteritems():
    ...
    summaryYc = 4 * subYc[smallIdx].sum() + subYc[np.logical_not(smallIdx)].sum() + outerYc.sum()
    outerSize = 10
    ...
    summaryPlots[id]['N'] = 4 * smallIdx.sum() + (subYc.__len__() - smallIdx.sum()) + outerYc.__len__()


# write out summary ground truth for each plot
outFileName = 'C:\Data\Development\Projects\PhD GeoInformatics\Data\GEF Sampling\Sampling March 2018\Summary - Woody.csv'
with open(outFileName, 'wb') as outfile:
