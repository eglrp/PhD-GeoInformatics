...
% 
%   H = SCATTERD(A)
%   H = SCATTERD(A,DIM,S,CMAP,FONTSIZE,'label','both','legend','gridded','ident')
%
% INPUT
%   A     Dataset or matrix
...
% $Id: scatterd.m,v 1.3 2007/01/02 14:50:36 davidt Exp $

% REVISIONS
% DR1 - Dick, 05-10-2004
% Added plotting of unlabeled data as 'k.'.

function [handle, scaleLims] = scatterd2(a,p1,p2,p3,p4,p5,p6,p7,p8,p9)
	...
	if (nargin < 10), par{9} = []; else, par{9} = p9; end
	...
	for i = 1:5
		if (~isempty(par{i})) 
			...
			elseif (strcmp(par{i},'ident'))
				plotident = 1; plotsym = 1;
			elseif (strcmp(par{i},'images'))
				plotim = 1; images = par{i+1};
                plotsym = 1;
			elseif (strcmp(par{i},'scale'))
				scaleLims = par{i+1};
            elseif ~isdataset(par{i}) && ischar(par{i})
				s	= par{i};
			end
		end
	end
	...
	if (isempty(s))
		...
		if (str2num(vers(1)) < 5)
			...
			sym = ['+*oxsdv^<>p']';
			i   = [1:44];
			ss  = [col(i-floor((i-1)/(length(col)))*length(col))' sym(i-floor((i-1)/11)*11)];
		end
  ...
  % Also plot label "0" (unlabeled).
	for i = 0:c
		...
		elseif ((d == 2) & ~isempty(J))
			...
			if (plotim)
                imsz = getfeatsize(images);
                if (~isempty(scaleLims))
                    oy = scaleLims;
                else
                    oy = 0.05*(max(a)-min(a));
                    scaleLims = oy;
				...
				for j = 1:length(J)
					...
					h1 = imagesc([a(J(j),1)-oy(1)/2 a(J(j),1)+oy(1)/2], [a(J(j),2)-oy(2)/2 a(J(j),2)+oy(2)/2], im);
                    h = rectangle('EdgeColor', symbol(1:1), 'Position', [a(J(j),1)-0.51*oy(1) a(J(j),2)-0.51*oy(2) 1.02*oy(1) 1.02*oy(2)]);
					thandle = [thandle h];
                    if (size(im, 3)==1)
                        colormap gray;
                    end
			...
			if (plotident)
				...
				for j = 1:length(J)
                    if (~plotim)
    					h = text(a(J(j),1)+oy(1),a(J(j),2)+oy(2),id{J(j)},'color', symbol(1:1));
                    else
    					h = text(a(J(j),1) - 0.45*oy(1), a(J(j),2), id{J(j)},'color', symbol(1:1));
                    end
					set(h,'fontsize',lab_size/2);
					thandle = [thandle h]; if (~isempty(map)), set (h, 'color', map(i+1,:)); end
				end
			end
		elseif (~isempty(J))
	...
	if (plotsym)
		...
		if (plotlegend), 
 			[ht, hl] = legend(lhandle(:), strrep(cellstr(lablist), '_','\_'));
%  			[ht, hl] = legend(lhandle(:), cellstr(lablist));
			hl = hl(:)';
	...
	if (~gridrun) & (length(get(gcf,'children')) == 1 & any(get(gca,'position')>0.80))
		set(gca,'position',[0.10 0.13 0.82 0.78]); % make axis labels visible
	end
