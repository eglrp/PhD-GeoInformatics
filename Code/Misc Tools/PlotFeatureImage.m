function [Spectra IjLocs] = PlotFeatureImage(Image, FeatureLabels)
...
% loHi = stretchlim(dImage(1:100:end)', [0.01 0.99]);

dImage = double(Image);

for i = 1:size(Image, 3)
    band = Image(:,:,i);
    dBand = double(band - min(band(:)));
    dBand = dBand./max(dBand(:));
    loHi = stretchlim(dBand(1:100:end)', [0.05 0.95]);
    dBand = imadjust(dBand, loHi);
    dImage(:,:,i) = dBand;
end
...
% return
if (size(Image, 1)*1.5 >= size(Image, 2))
    subplot(1, 2, 1);
else
    subplot(2, 1, 1);
end
imH = imagesc(dImage(:,:,1));
...
%         Spectra(markerCount, :) = squeeze(Image(round(y), round(x), :))';
    ...
    elseif (but >= uint8('1') && but <= uint8(num2str(min([size(Image, 3) 9]))))
        ...
        title(imAxH, FeatureLabels{band});
    elseif (but == uint8('+'))
        band = band + 1;
